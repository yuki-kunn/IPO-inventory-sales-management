...
<div class="min-h-screen bg-background">
  <div class="max-w-7xl mx-auto p-4 sm:p-6 md:p-8 space-y-6 sm:space-y-8">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between border-b border-border pb-4 sm:pb-6 gap-4 ml-14 sm:ml-0">
      <div>
        <h1 class="text-2xl sm:text-3xl font-semibold tracking-tight">AirREGI 在庫・売上管理</h1>
        <p class="text-muted-foreground mt-1 text-xs sm:text-sm">原材料在庫と売上データを統合管理</p>
      </div>
      <div class="flex items-center gap-2">
        ...
        <Button variant="outline" onclick={() => (showUploader = !showUploader)} class="touch-manipulation">
    ...
    {#if lowStockIngredients.length > 0}
      <Card class="border-yellow-200 dark:border-yellow-800 bg-yellow-50 dark:bg-yellow-950/20">
        <CardHeader>
          <CardTitle class="text-yellow-800 dark:text-yellow-200 flex items-center gap-2">
    ...
    <Card>
      ...
      <CardContent>
        <IngredientsTable ingredients={currentIngredients} onEdit={handleEdit} onDelete={handleDelete} />
      </CardContent>
    </Card>
  </div>
</div>

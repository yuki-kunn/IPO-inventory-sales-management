...
function createDailySalesFirestoreStore() {
  ...
  return {
    subscribe,
    addOrUpdate: async (salesDate: string, salesData: SalesData[], unregisteredCount: number = 0) => {
      if (!browser || !db) {
      ...
      try {
        console.log('[DailySales] 日別売上を追加/更新:', salesDate, '件数:', salesData.length, '未登録:', unregisteredCount);
    ...
    markAsProcessed: async (date: string, unregisteredCount: number, processedProducts: string[]) => {
      if (!browser || !db) {
      ...
      try {
        console.log('[DailySales] 処理済みとしてマーク:', date, '処理商品数:', processedProducts.length);

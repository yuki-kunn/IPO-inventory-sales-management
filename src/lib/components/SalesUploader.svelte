  ...
  async function processFile(file: File) {
    try {
      ...
      if (result.success && result.salesData.length > 0) {
        ...
        await dailySales.addOrUpdate(result.salesDate, result.salesData, processResult.totalUnregistered);

        // 処理済み商品リストを作成
        const processedProductNames = processResult.processedProducts.map(p => p.productName);

        // 処理済みとしてマーク
        await dailySales.markAsProcessed(result.salesDate, processResult.totalUnregistered, processedProductNames);
  ...
  <CardContent>
    <div class="space-y-4">
      ...
      {#if batchResults.length > 0}
        <div class="rounded-md p-4 bg-blue-500/10 border border-blue-500/20">
          ...
          <div class="space-y-2 max-h-64 overflow-y-auto">
            {#each batchResults as result}
              <div class="text-xs p-2 rounded {result.success ? 'bg-green-500/10' : 'bg-red-500/10'}">
                <div class="flex items-center justify-between">
                  <span class="font-medium {result.success ? 'text-green-700 dark:text-green-300' : 'text-red-700 dark:text-red-300'}">
                    {result.date}
                  </span>
                ...
                {#if result.success}
                  <div class="mt-1 text-muted-foreground space-x-3">
                    <span>商品: {result.imported}件</span>
                    <span>処理: {result.processed}件</span>
                    {#if result.unregistered > 0}
                      <span class="text-yellow-600 dark:text-yellow-400">未登録: {result.unregistered}件</span>
                    {/if}
                  </div>
                {/if}
              </div>
            ...
            {#if processResult}
              <div class="mt-3 pt-3 border-t border-green-500/20">
                ...
                {#if processResult.processedProducts.length > 0}
                  <ul class="text-xs text-green-700 dark:text-green-300 space-y-1 max-h-40 overflow-y-auto">
      ...
      <div class="text-xs text-muted-foreground space-y-2">
        <div>
          <p class="font-medium mb-1">アップロード方法:</p>
          <ul class="text-[10px] space-y-1 ml-4 list-disc">
            <li><strong>ファイル選択</strong>: 単一ファイルまたは複数ファイルを選択</li>
            <li><strong>フォルダ選択</strong>: フォルダ内のすべてのCSVファイルを一括アップロード</li>
          </ul>
        </div>
        <div>
          <p class="font-medium mb-1">対応フォーマット:</p>
          <code class="block bg-muted p-2 rounded text-[10px] leading-relaxed">
            商品名,カテゴリー,税区分,販売総売上,構成比%,粗利総額,構成比%,販売商品数,構成比%,返品商品数,構成比%,商品ID,商品コード,バーコード
          </code>
        </div>
        <p class="text-[10px]">
          ※ Shift-JIS/UTF-8対応 | ファイル名から日付を自動抽出（例: 商品別売上_20260413-20260413.csv）
        </p>
      </div>
    </div>
  </CardContent>
